...
public class MemberServiceImpl implements MemberService {
	...
	public Member login(String id, String password) throws Exception {
		...
		if(member==null) throw new Exception("로그인 오류");
		
		//DB로부터 password 일치여부 확인
		if(!member.getPassword().equals(password)) throw new Exception("로그인 오류");
			return member;
	}
	...
	Member getKakaoUserInfo(String token) throws Exception {
		...
		Member member= new Member.Builder(1,id,nickname, profile_image).build();
